...
def main():
    ...
    try:
        if update_file.endswith('.exe'):
            ...
            # For macOS dmg: attach, copy, detach
            if sys.platform == "darwin":
                ...
                if app_bundle_path.endswith('.app'):
                    result = subprocess.run(['hdiutil', 'attach', '-nobrowse', update_file], capture_output=True, text=True)
                    mount_point = None
                    ...
                    if mount_point:
                        ...
                        if new_app_path:
                            print(f"Found app at {new_app_path}, replacing...")
                            shutil.rmtree(app_bundle_path, ignore_errors=True)
                            shutil.copytree(new_app_path, app_bundle_path)
                            ...
                            print("Could not find .app in DMG")
                            subprocess.call(['hdiutil', 'detach', mount_point, '-force'])
                else:
