...
class GlossaryManagerPage(ShellPage):
    ...
    ENTRY_DATA_ROLE = Qt.ItemDataRole.UserRole + 3  # Хранит entry_data для ленивого создания кнопок
    ...
    TABLE_ROW_HEIGHT = 36
    VISIBLE_WIDGET_BUFFER = 15  # Буфер строк выше/ниже видимой области для предзагрузки виджетов
    ...
    def __init__(self, parent=None, mode='standalone', project_path=None):
        ...
        self._versions_cache = None  # Кэш glossary_versions.json (set ключей или None)
    ...
    def init_ui(self):
        ...
        for i in range(3): header.setSectionResizeMode(i, QHeaderView.ResizeMode.Interactive)
        for i in range(3, 5): header.setSectionResizeMode(i, QHeaderView.ResizeMode.Fixed)
        
        def table_resize_event(event):
            QTableWidget.resizeEvent(self.table, event)
            width = self.table.viewport().width()
            btns_width = self.table.columnWidth(3) + self.table.columnWidth(4)
            if btns_width <= 0: btns_width = 80
            col_w = max(10, (width - btns_width) // 3)
            self.table.setColumnWidth(0, col_w)
            self.table.setColumnWidth(1, col_w)
            self.table.setColumnWidth(2, col_w)
            
        self.table.resizeEvent = table_resize_event
        ...
        # --- Ленивое создание виджетов-кнопок (только для видимых строк) ---
        self._visible_widget_rows = set()
        self._scroll_widget_timer = QTimer(self)
        self._scroll_widget_timer.setSingleShot(True)
        self._scroll_widget_timer.setInterval(30)  # 30ms дебаунс при скролле
        self._scroll_widget_timer.timeout.connect(self._update_visible_row_widgets)
    ...
    def _add_table_row(self, entry_data: dict):
        ...
        self._update_visible_row_widgets()  # Создать кнопки для видимых строк
    ...
    def _populate_table_row(self, row, entry_data: dict):
        """Заполняет одну строку таблицы данными. Кнопки создаются лениво."""
        items = [
            QTableWidgetItem(str(entry_data.get(k, '')))
            for k in ['original', 'rus', 'note']
        ]
        items[0].setData(self.DB_ID_ROLE, entry_data['id'])
        items[0].setData(self.ENTRY_DATA_ROLE, entry_data)  # Сохраняем для ленивого создания кнопок
        for col, item in enumerate(items):
            self.table.setItem(row, col, item)
        # Кнопки НЕ создаются здесь — они создаются лениво в _update_visible_row_widgets
...
# ---------------------------------------------------------------------------
    ...
    def _load_current_page(self):
        ...
        self.table.blockSignals(True)
        self.table.setUpdatesEnabled(False)  # Блокируем перерисовку Qt
        
        # Предзагружаем кэш версий один раз для всей страницы
        ...
        self.table.setUpdatesEnabled(True)  # Разблокируем перерисовку Qt
        self.table.blockSignals(False)
        self._update_pagination_controls()
        ...
        # Создаём кнопки только для видимых строк (ленивый рендеринг)
        self._visible_widget_rows = set()
        self._update_visible_row_widgets()
    ...
    def _apply_highlights_chunk(self):
        ...
        # Используем conflict_map напрямую — не нужен запрос к БД
        ...
        while self._highlight_row_index < self.table.rowCount() and rows_processed < CHUNK_SIZE:
            ...
            if item:
                ...
                conflict_types = conflict_map.get(original_text, set())
    ...
    def _on_table_scroll(self):
        """Дебаунс-обработчик скролла таблицы. Запускает обновление виджетов."""
        self._scroll_widget_timer.start()

    def _update_visible_row_widgets(self):
        """
        Создаёт виджеты-кнопки ТОЛЬКО для видимых строк + буфер.
        Удаляет виджеты для строк, ушедших за пределы видимости.
        Это устраняет лаг при скролле: вместо N виджетов — только ~30-40.
        """
        row_count = self.table.rowCount()
        if row_count == 0:
            self._visible_widget_rows = set()
        ...
        last_visible = self.table.rowAt(viewport.height())
        ...
        buf = self.VISIBLE_WIDGET_BUFFER
        start = max(0, first_visible - buf)
        end = min(row_count - 1, last_visible + buf)

        needed_rows = set(range(start, end + 1))

        # Удаляем виджеты из строк, ушедших из зоны видимости
        rows_to_remove = self._visible_widget_rows - needed_rows
        for row in rows_to_remove:
            self.table.removeCellWidget(row, 3)
            self.table.removeCellWidget(row, 4)

        # Создаём виджеты для новых видимых строк
        rows_to_add = needed_rows - self._visible_widget_rows
        for row in rows_to_add:
            item = self.table.item(row, 0)
            if item:
                entry_data = item.data(self.ENTRY_DATA_ROLE)
                if entry_data:
                    self._create_row_buttons(row, entry_data)

        self._visible_widget_rows = needed_rows

    def _load_versions_cache(self):
        """Загружает glossary_versions.json один раз и кэширует набор ключей."""
        if self._versions_cache is not None:
            return  # Кэш уже загружен
        
        if not self.associated_project_path:
            self._versions_cache = set()
            return
        ...
        try:
            with open(v_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
                self._versions_cache = set(data.keys()) if isinstance(data, dict) else set()
        except Exception:
            self._versions_cache = set()

    def _check_if_term_has_versions(self, term):
        """Быстрая проверка наличия версий через кэш (O(1) вместо чтения файла)."""
        if self._versions_cache is None:
            self._load_versions_cache()
        return term in self._versions_cache
    ...
    def _open_versioning_dialog(self, item_dict):
        ...
        # Инвалидируем кэш версий, чтобы он перечитался при загрузке страницы
