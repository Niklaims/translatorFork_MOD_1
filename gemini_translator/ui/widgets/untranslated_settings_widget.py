# -*- coding: utf-8 -*-
"""Виджет настроек и промпта для этапа 'Доперевод недоперевода (AI)'."""
...
class UntranslatedSettingsWidget(QWidget):
    """Вкладка управления параметрами и промптом доперевода недопереведенных фрагментов."""

    settings_changed = pyqtSignal()
    open_interactive_requested = pyqtSignal()
    ...
    def _init_ui(self):
        ...
        title_label = QLabel("🔍 Доперевод недоперевода (AI)")
        ...
        subtitle_label = QLabel(
            "Настройка правил, системного промпта и параметров автоматического устранения "
            "непереведенных фрагментов (иероглифов CJK, латиницы, химер) в главах книги."
        )
        ...
        # 2. Блок параметров выполнения
        params_group = QGroupBox("Параметры доперевода")
        params_layout = QVBoxLayout(params_group)
        params_layout.setSpacing(10)

        # Модель и провайдер
        model_row = QHBoxLayout()
        model_row.addWidget(QLabel("Модель для доперевода:"))
        ...
        model_row.addSpacing(15)
        model_row.addWidget(QLabel("Температура:"))
        self.temp_spin = NoScrollDoubleSpinBox()
        self.temp_spin.setRange(0.0, 1.5)
        self.temp_spin.setSingleStep(0.1)
        self.temp_spin.setValue(0.3)
        self.temp_spin.setToolTip("Низкая температура (0.2–0.4) повышает точность и верность контексту.")
        self.temp_spin.valueChanged.connect(self._on_setting_changed)
        model_row.addWidget(self.temp_spin)

        model_row.addSpacing(15)
        model_row.addWidget(QLabel("Размер чанка (слов):"))
        self.chunk_size_spin = NoScrollSpinBox()
        self.chunk_size_spin.setRange(1, 50)
        self.chunk_size_spin.setValue(10)
        self.chunk_size_spin.setToolTip("Количество непереведенных фрагментов, отправляемых в одном запросе к ИИ.")
        self.chunk_size_spin.valueChanged.connect(self._on_setting_changed)
        model_row.addWidget(self.chunk_size_spin)

        params_layout.addLayout(model_row)

        # Чекбоксы сканирования
        checks_row = QHBoxLayout()
        self.scan_cjk_chk = QCheckBox("Искать и переводить CJK-иероглифы (китайский, японский, корейский)")
        self.scan_cjk_chk.setChecked(True)
        self.scan_cjk_chk.toggled.connect(self._on_setting_changed)
        checks_row.addWidget(self.scan_cjk_chk)

        self.scan_latin_chk = QCheckBox("Искать и адаптировать латинские включения")
        self.scan_latin_chk.setChecked(True)
        self.scan_latin_chk.toggled.connect(self._on_setting_changed)
        checks_row.addWidget(self.scan_latin_chk)

        self.auto_apply_chk = QCheckBox("Автоматически применять правки в главы")
        self.auto_apply_chk.setChecked(True)
        self.auto_apply_chk.toggled.connect(self._on_setting_changed)
        checks_row.addWidget(self.auto_apply_chk)

        checks_row.addStretch()
        params_layout.addLayout(checks_row)

        main_layout.addWidget(params_group)

        # 3. Редактор промпта доперевода
        prompt_group = QGroupBox("Промпт доперевода")
        prompt_layout = QVBoxLayout(prompt_group)
        prompt_layout.setContentsMargins(5, 5, 5, 5)

        self.preset_widget = PresetWidget(
            parent=self,
            preset_name="Промпт доперевода",
        ...
        self.btn_open_interactive = QPushButton("🔍 Открыть интерактивный помощник исправления недопереводов…")
        self.btn_open_interactive.setToolTip("Запустить графический инструмент для детального просмотра и ручной проверки кандидатов")
        self.btn_open_interactive.clicked.connect(self.open_interactive_requested.emit)
        bottom_layout.addWidget(self.btn_open_interactive)
    ...
    def get_settings(self) -> dict:
        return {
            ...
            "scan_cjk": self.scan_cjk_chk.isChecked(),
            "scan_latin": self.scan_latin_chk.isChecked(),
            "auto_apply": self.auto_apply_chk.isChecked(),
            "prompt": self.preset_widget.get_prompt() or api_config.default_untranslated_prompt(),
    ...
    def set_settings(self, data: dict):
        ...
        if "scan_cjk" in data:
            self.scan_cjk_chk.setChecked(bool(data["scan_cjk"]))
        if "scan_latin" in data:
            self.scan_latin_chk.setChecked(bool(data["scan_latin"]))
    ...
    def _load_saved_settings(self):
        saved = self.settings_manager._generic_loader("untranslated_fixing_settings", {})
        if saved:
            self.set_settings(saved)

    def _save_settings(self):
        self.settings_manager._generic_saver("untranslated_fixing_settings", self.get_settings())
    ...
    def set_session_mode(self, is_session_active: bool):
        for widget in [self.model_combo, self.temp_spin, self.chunk_size_spin,
                       self.scan_cjk_chk, self.scan_latin_chk, self.auto_apply_chk]:
