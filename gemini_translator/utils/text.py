...
def attribution_merge_replacer(match: re.Match) -> str:
    ...
    После слова или закрывающей кавычки ставит запятую («— Я хотел, — сказал он»),
...
def oper_dash_symbol(html_content: str) -> str:
    ...
    # (?<![.…]) — только начало серии. Совпадений это не меняет: если серия
    # подходит с середины, она тем более подходит на символ левее, и левое
    # совпадение находится первым. Зато без этого движок на каждой позиции
    # внутри длинной череды точек проходил её заново, и зацикленный ответ
    # модели на 60 КБ точек обрабатывался 29 секунд вместо миллисекунд.
