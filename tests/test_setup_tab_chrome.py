...
def test_translator_main_tabs_are_stacked_widget():
    setup_source = Path("gemini_translator/ui/dialogs/setup.py").read_text(encoding="utf-8")

    assert "self.tabs_group = QStackedWidget()" in setup_source
...
def test_bottom_action_bar_keeps_standard_buttons():
    ...
    assert 'self.use_project_settings_btn = QtWidgets.QPushButton("Режим: Глобальные настройки")' in setup_source
