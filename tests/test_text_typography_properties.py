...
#
# refine_typography_in_html: вложенные кавычки перебирают иерархию по кругу, а
# глубину считают только по «»: при 11 незакрытых « второй проход меняет
# последнюю. Так же ведёт себя тире с пробелом перед вложенной закрывающей
# кавычкой: первый проход делает из «»» “, и второй уже разносит тире. Оба
# случая — битые или глубоко вложенные кавычки; в приложении функцию зовёт
# только prettify_html, уже после repair_quotes.
#
...
def test_html_transform_is_idempotent(name):
    ...
    # Входы, которые hypothesis нашёл для refine_typography_in_html (см.
    # CONVERGING_TRANSFORMS). Остальные функции обязаны выдержать их сразу.
...
def test_transform_reaches_a_fixed_point(name):
    ...
    @PROPERTY_SETTINGS
    @given(value=html_fragment())
    # Входы, которые hypothesis нашёл для refine_typography_in_html. Случайный
    # генератор находит такие редко, поэтому они заданы явно.
    @example(value="<p>«</p><em>–</em><em>–</em>")
    @example(value="<p>«««</p><p>««««</p><p>««««</p>")
    def check(value):
        seen = []
